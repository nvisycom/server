...
account/workspace avatars) maps to a key prefix within it. Backed by the AWS S3
SDK, so it works against AWS S3 or any S3-compatible server (RustFS, MinIO,
Cloudflare R2, …) selected by endpoint.
