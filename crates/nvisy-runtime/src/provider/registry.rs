...
impl CredentialsRegistry {
    ...
    pub fn new(value: serde_json::Value) -> Result<Self> {
        let map: HashMap<Uuid, ProviderCredentials> =
            serde_json::from_value(value).map_err(Error::CredentialsRegistry)?;
        Ok(Self { credentials: map })
    }

    /// Retrieves credentials by ID.
    pub fn get(&self, credentials_id: Uuid) -> Result<&ProviderCredentials> {
        self.credentials
            .get(&credentials_id)
            .ok_or(Error::CredentialsNotFound(credentials_id))
    }
    ...
    pub fn insert(&mut self, credentials: ProviderCredentials) -> Uuid {
        ...
        loop {
            let id = Uuid::new_v4();
            if let Entry::Vacant(entry) = self.credentials.entry(id) {
