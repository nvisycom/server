...
today, with Dropbox, OneDrive, and Box to follow — behind a single
`FileServiceClient`. It owns the OAuth2 authorization-code flow (with
refresh), driven through the platform's shared `reqwest` client, and the
per-provider REST calls to list and stream files in and out. Sibling to
`nvisy-object-store`, which covers object stores (S3, Azure, GCS) with
static credentials.
