...
impl RetrievedChunk {
    ...
    pub fn estimated_tokens(&self) -> u32 {
        self.content
            .as_ref()
            .map(|c| c.len() as u32 / 4) // Rough estimate: ~4 chars per token
            .unwrap_or(self.metadata.content_len() / 4)
    }
}
...
mod tests {
    ...
    #[test]
    fn chunk_metadata_location() {
        let meta = ChunkMetadata::new(2, 1000, 2000)
            .with_page(5)
            .with_section("Introduction");

        let location = meta.location_string();
        assert!(location.contains("page 5"));
        assert!(location.contains("Introduction"));
        assert!(location.contains("chunk 3"));
    }

    #[test]
    fn chunk_metadata_byte_range() {
        let meta = ChunkMetadata::new(0, 100, 500);
        assert_eq!(meta.byte_range(), 100..500);
        assert_eq!(meta.content_len(), 400);
    }

    #[test]
    fn chunk_metadata_from_json() {
        ...
        let meta = ChunkMetadata::from_json(&json, 5);
        assert_eq!(meta.start_offset, 100);
        assert_eq!(meta.end_offset, 500);
        assert_eq!(meta.page, Some(3));
        assert_eq!(meta.section, Some("Methods".to_string()));
        assert_eq!(meta.chunk_index, 5);
    }

    #[test]
    fn retrieved_chunk_without_content() {
        ...
        assert!(chunk.content_or_placeholder().contains("not yet loaded"));
    }

    #[test]
    fn retrieved_chunk_with_content() {
        let chunk = RetrievedChunk::new(
            Uuid::nil(),
            Uuid::nil(),
            0.85,
        ...
        assert!(chunk.has_content());
        assert_eq!(chunk.content_or_placeholder(), "Hello, world!");
    }
}
