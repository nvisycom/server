...
/// Tracing target for OCR model operations.
///
/// Use this target for logging model selection, configuration, and model-related errors.
pub const TRACING_TARGET_MODELS: &str = "nvisy_olmocr2::models";

/// Tracing target for OCR processing operations.
///
/// Use this target for logging document processing, text extraction, and processing-related errors.
pub const TRACING_TARGET_PROCESSING: &str = "nvisy_olmocr2::processing";

/// Tracing target for OCR results operations.
///
/// Use this target for logging result parsing, confidence scoring, and result-related errors.
pub const TRACING_TARGET_RESULTS: &str = "nvisy_olmocr2::results";

/// Tracing target for OCR API communication.
///
/// Use this target for logging API requests, responses, and network-related errors.
pub const TRACING_TARGET_API: &str = "nvisy_olmocr2::api";
...
pub use client::{OcrClient, OcrConfig, OcrCredentials};
    ...
    pub fn user_message(&self) -> String {
        match self {
            ...
            Error::Timeout { timeout } => {
                format!("OCR processing timed out after {:?}. Please try again with a smaller document.", timeout)
            }
            ...
            Error::LowConfidence {
                ...
                format!("OCR confidence ({:.1}%) is below the minimum threshold ({:.1}%). Results may be inaccurate.",
