...
use nvisy_postgres::query::{WorkspaceIntegrationRepository, WorkspaceIntegrationRunRepository};

use crate::extract::{AuthProvider, AuthState, Json, Path, Permission, Query, ValidateJson};
use crate::handler::request::{
    CreateIntegration, CursorPagination, IntegrationPathParams, IntegrationRunPathParams,
    UpdateIntegration, UpdateIntegrationCredentials, WorkspacePathParams,
};
use crate::handler::response::{
    ErrorResponse, Integration, IntegrationRun, IntegrationRunsPage, IntegrationsPage,
};
...
async fn list_integration_runs(
    State(pg_client): State<PgClient>,
    AuthState(auth_state): AuthState,
    Path(path_params): Path<WorkspacePathParams>,
    Query(pagination): Query<CursorPagination>,
...
fn list_integration_runs_docs(op: TransformOperation) -> TransformOperation {
    op.summary("List integration runs")
...
fn get_integration_run_docs(op: TransformOperation) -> TransformOperation {
...
pub fn routes() -> ApiRouter<ServiceState> {
    ...
    ApiRouter::new()
        ...
        .api_route(
            "/workspaces/{workspaceId}/integration-runs/",
            get_with(list_integration_runs, list_integration_runs_docs),
        )
        ...
        // Integration run routes
        .api_route(
            "/integration-runs/{runId}/",
            get_with(get_integration_run, get_integration_run_docs),
        )
