    ...
    fn from(c: DocumentConstraints) -> Self {
        match c {
            DocumentConstraints::DisplayNameLengthMin => ErrorKind::BadRequest
                .with_context("Document name must be at least 3 characters long"),
            DocumentConstraints::DisplayNameLengthMax => {
                ErrorKind::BadRequest.with_context("Document name cannot exceed 240 characters")
            }
            DocumentConstraints::DescriptionLengthMax => {
                ErrorKind::BadRequest.with_context("Document description is too long")
            }
            DocumentConstraints::TagsCountMax => {
                ErrorKind::BadRequest.with_context("Too many tags")
            }
            DocumentConstraints::MetadataSizeMin => ErrorKind::InternalServerError.into_error(),
            DocumentConstraints::MetadataSizeMax => {
                ErrorKind::BadRequest.with_context("Document metadata exceeds maximum allowed size")
            }
            DocumentConstraints::SettingsSizeMin => ErrorKind::InternalServerError.into_error(),
            DocumentConstraints::SettingsSizeMax => {
                ErrorKind::BadRequest.with_context("Document settings exceed maximum allowed size")
            }
    ...
    fn from(c: DocumentFileConstraints) -> Self {
        match c {
            DocumentFileConstraints::DisplayNameLengthMin => {
                ErrorKind::BadRequest.with_context("File name must be at least 2 characters long")
            }
            DocumentFileConstraints::DisplayNameLengthMax => {
                ErrorKind::BadRequest.with_context("File name cannot exceed 240 characters")
            }
            DocumentFileConstraints::OriginalFilenameLengthMin => {
                ErrorKind::BadRequest.with_context("Original filename is too short")
            }
            DocumentFileConstraints::OriginalFilenameLengthMax => {
                ErrorKind::BadRequest.with_context("Original filename is too long")
            }
            DocumentFileConstraints::FileExtensionFormat => {
                ErrorKind::BadRequest.with_context("Invalid file extension format")
            }
            DocumentFileConstraints::MimeTypeLengthMin => {
                ErrorKind::BadRequest.with_context("MIME type is too short")
            }
            DocumentFileConstraints::MimeTypeLengthMax => {
                ErrorKind::BadRequest.with_context("MIME type is too long")
            }
            DocumentFileConstraints::ProcessingPriorityMin => {
                ErrorKind::BadRequest.with_context("Processing priority is too low")
            }
            DocumentFileConstraints::ProcessingPriorityMax => {
                ErrorKind::BadRequest.with_context("Processing priority is too high")
            }
            DocumentFileConstraints::ProcessingAttemptsMin => {
                ErrorKind::InternalServerError.into_error()
            }
            DocumentFileConstraints::ProcessingAttemptsMax => {
                ErrorKind::BadRequest.with_context("Too many processing attempts")
            }
            DocumentFileConstraints::ProcessingErrorLengthMax => {
                ErrorKind::BadRequest.with_context("Processing error message is too long")
            }
            DocumentFileConstraints::ProcessingDurationMin => {
                ErrorKind::InternalServerError.into_error()
            }
            DocumentFileConstraints::FileSizeMin => {
                ErrorKind::BadRequest.with_context("File size must be greater than 0")
            }
            ...
            DocumentFileConstraints::MetadataSizeMax => {
                ErrorKind::BadRequest.with_context("File metadata is too large")
            }
            ...
            DocumentFileConstraints::RetentionPeriodMin => ErrorKind::BadRequest
                .with_context("File retention period must be at least 1 second"),
            DocumentFileConstraints::RetentionPeriodMax => {
                ErrorKind::BadRequest.with_context("File retention period cannot exceed 7 days")
            }
    ...
    fn from(c: DocumentVersionConstraints) -> Self {
        match c {
            DocumentVersionConstraints::VersionNumberMin => {
                ErrorKind::BadRequest.with_context("Version number must be at least 1")
            }
            DocumentVersionConstraints::DisplayNameLengthMin => ErrorKind::BadRequest
                .with_context("Version name must be at least 2 characters long"),
            DocumentVersionConstraints::DisplayNameLengthMax => {
                ErrorKind::BadRequest.with_context("Version name cannot exceed 240 characters")
            }
            DocumentVersionConstraints::FileExtensionFormat => {
                ErrorKind::BadRequest.with_context("Invalid file extension format")
            }
            DocumentVersionConstraints::MimeTypeNotEmpty => {
                ErrorKind::BadRequest.with_context("MIME type cannot be empty")
            }
            ...
            DocumentVersionConstraints::ProcessingResultsSizeMax => {
                ErrorKind::BadRequest.with_context("Processing results are too large")
            }
            DocumentVersionConstraints::MetadataSizeMin => {
                ErrorKind::InternalServerError.into_error()
            }
            DocumentVersionConstraints::MetadataSizeMax => {
                ErrorKind::BadRequest.with_context("Version metadata is too large")
            }
            DocumentVersionConstraints::RetentionPeriodMin => ErrorKind::BadRequest
                .with_context("Version retention period must be at least 1 second"),
            DocumentVersionConstraints::RetentionPeriodMax => {
                ErrorKind::BadRequest.with_context("Version retention period cannot exceed 7 days")
            }
            ...
            DocumentVersionConstraints::UniqueVersion => {
                ErrorKind::Conflict.with_context("A version with this number already exists")
            }
        }
    }
}
