...
//!
//! Connections store a cron expression; the scheduler asks whether a connection
//! is due relative to its last successful sync. [`StandardCronSchedule`] answers
//! that with croner. croner operates on `chrono` datetimes, so this module
//! bridges jiff timestamps to chrono at the Unix-second boundary and keeps that
//! conversion isolated here.
...
/// Decides when a cron-scheduled connection is due to sync, and validates cron
/// expressions. Backed by croner; carries no state.
#[derive(Debug, Clone, Copy, Default)]
pub struct StandardCronSchedule;

impl StandardCronSchedule {
...
mod tests {
    ...
    fn every_minute_due_after_a_minute() {
        ...
        assert!(StandardCronSchedule.is_due("* * * * *", Some(ts(0)), ts(120)));
    }
    ...
    fn not_due_before_next_occurrence() {
        ...
        assert!(!StandardCronSchedule.is_due("0 0 * * *", Some(last), ts(30)));
    }
    ...
    fn never_synced_is_due_once_first_occurrence_passed() {
        ...
        // fired -> due.
        assert!(StandardCronSchedule.is_due("* * * * *", None, ts(120)));
    }
