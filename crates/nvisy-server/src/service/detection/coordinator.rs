//! In-process wake coordination for the detection pipeline.

use std::sync::Arc;

use tokio::sync::Notify;

/// The rendezvous between the detection-job enqueue path and the outbox drainer.
///
/// After a handler commits a new detection job, it calls [`wake`](Self::wake) so
/// the drainer drains the job at once instead of waiting for its next timer tick;
/// the drainer awaits [`notified`](Self::notified) alongside that timer. The wake
/// is in-process only and best-effort — the drainer's timer remains the
...
/// [`Notify`]) between the [`DetectionQueue`](super::DetectionQueue) that
