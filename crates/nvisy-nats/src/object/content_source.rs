...
impl ContentSource {
    ...
    ///
    /// ```
    /// use nvisy_nats::object::ContentSource;
    ///
    ...
    /// assert!(timestamp > 1577836800000); // After Jan 1, 2020
    /// ```
    #[must_use]
...
mod tests {
    ...
    fn test_serde_serialization() {
        ...
        // Verify that it serializes as a simple UUID string, not a JSON object
        assert!(
        ...
        assert!(
            !serialized.starts_with('{'),
            "Should not serialize as a JSON object"
        );

        // The serialized string should be a valid UUID when unquoted
        let uuid_str = serialized.trim_matches('"');
        assert!(
