...
impl ProjectRepository {
    ...
    /// Finds projects created from a specific template.
    pub async fn find_projects_by_template(
        conn: &mut AsyncPgConnection,
        ...
        let project_list = projects
            .filter(template_id.eq(template_project_id))
            .filter(deleted_at.is_null())
            .select(Project::as_select())
            .order(created_at.desc())
            .limit(pagination.limit)
    ...
    /// Finds projects by category.
    pub async fn find_projects_by_category(
        conn: &mut AsyncPgConnection,
        category_name: &str,
        pagination: Pagination,
    ) -> PgResult<Vec<Project>> {
