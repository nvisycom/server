//! Assistant-job outbox repository: the write side (insert in the create-comment
...
//! transaction per batch (see the assistant-job drainer), so the `FOR UPDATE SKIP
...
use crate::model::{NewWorkspaceAssistantJob, WorkspaceAssistantJob};
use crate::types::OutboxStatus;
use crate::{Error, PgConnection, Result, schema};

/// Read and write operations on the assistant-job outbox.
pub trait AssistantJobOutboxRepository {
    /// Inserts one job outbox row. Called in the same transaction as the comment
    /// it queues, so the two commit atomically.
    fn insert_assistant_job(
        &mut self,
        row: NewWorkspaceAssistantJob,
    ) -> impl Future<Output = Result<WorkspaceAssistantJob>> + Send;
    ...
    fn claim_assistant_job_batch(
        &mut self,
        limit: i64,
    ) -> impl Future<Output = Result<Vec<WorkspaceAssistantJob>>> + Send;

    /// Marks a row processed (its job durably published), taking it out of the
    /// pending set. Runs in the drainer's batch transaction.
    fn mark_assistant_job_processed(&mut self, id: Uuid)
    ...
    fn defer_assistant_job_attempt(
        &mut self,
        id: Uuid,
        backoff_secs: i64,
    ) -> impl Future<Output = Result<()>> + Send;
    ...
    /// batch transaction.
    fn mark_assistant_job_failed(&mut self, id: Uuid) -> impl Future<Output = Result<()>> + Send;
}

impl AssistantJobOutboxRepository for PgConnection {
    async fn insert_assistant_job(
        &mut self,
        row: NewWorkspaceAssistantJob,
    ) -> Result<WorkspaceAssistantJob> {
        use schema::workspace_assistant_jobs;

        diesel::insert_into(workspace_assistant_jobs::table)
            .values(&row)
            .returning(WorkspaceAssistantJob::as_returning())
            .get_result(self)
            .await
            .map_err(Error::from)
    }

    async fn claim_assistant_job_batch(
        &mut self,
        limit: i64,
    ) -> Result<Vec<WorkspaceAssistantJob>> {
        use schema::workspace_assistant_jobs::{self, dsl};

        workspace_assistant_jobs::table
            ...
            .limit(limit)
            .select(WorkspaceAssistantJob::as_select())
            .for_update()
            .skip_locked()
            .load(self)
            .await
    ...
    async fn mark_assistant_job_processed(&mut self, id: Uuid) -> Result<()> {
        use schema::workspace_assistant_jobs::{self, dsl};

        diesel::update(workspace_assistant_jobs::table.filter(dsl::id.eq(id)))
            .set((
    ...
    async fn defer_assistant_job_attempt(&mut self, id: Uuid, backoff_secs: i64) -> Result<()> {
        use schema::workspace_assistant_jobs::{self, dsl};
    ...
    async fn mark_assistant_job_failed(&mut self, id: Uuid) -> Result<()> {
        use schema::workspace_assistant_jobs::{self, dsl};
...
mod tests {
    ...
    async fn reread(conn: &mut PgConn, id: Uuid) -> anyhow::Result<Option<WorkspaceAssistantJob>> {
        use schema::workspace_assistant_jobs::dsl;

        let row = dsl::workspace_assistant_jobs
            .filter(dsl::id.eq(id))
            .select(WorkspaceAssistantJob::as_select())
            .first(conn)
            .await
    ...
    async fn claim_then_process_removes_the_row_from_the_pending_set() -> anyhow::Result<()> {
        ...
        let comment_id = seed_comment(&db).await?;
        let mut conn = db.client.get_connection().await?;

        let job = conn
            .insert_assistant_job(NewWorkspaceAssistantJob::test(comment_id))
            .await?;

        let processed = conn
            .transaction(async |conn| -> Result<Uuid> {
                let batch = conn.claim_assistant_job_batch(10).await?;
                assert_eq!(batch.len(), 1);
                assert_eq!(batch[0].id, job.id);
                conn.mark_assistant_job_processed(batch[0].id).await?;
                Ok(batch[0].id)
            })
            .await?;
        ...
        let empty = conn
            .transaction(async |conn| conn.claim_assistant_job_batch(10).await)
            .await?;
    ...
    async fn defer_pushes_the_row_out_of_the_due_window() -> anyhow::Result<()> {
        ...
        conn.transaction(async |conn| -> Result<()> {
            let batch = conn.claim_assistant_job_batch(10).await?;
            assert_eq!(batch.len(), 1);
            conn.defer_assistant_job_attempt(batch[0].id, 3600).await?;
            Ok(())
        })
        .await?;
    ...
    async fn mark_failed_dead_letters_the_row() -> anyhow::Result<()> {
        ...
        conn.transaction(async |conn| -> Result<()> {
            let batch = conn.claim_assistant_job_batch(10).await?;
            conn.mark_assistant_job_failed(batch[0].id).await?;
            Ok(())
        })
        .await?;
