...
impl DocumentRepository {
    ...
    /// Gets comprehensive document statistics for a specific project.
    ///
    /// Calculates detailed metrics about document activity and engagement
    /// within a project workspace. This provides project managers and teams
    /// with insights into content creation patterns, collaboration levels,
    /// and overall project documentation health.
    ///
    /// # Arguments
    ///
    /// * `conn` - Active database connection for the query
    ...
    /// # Returns
    ///
    /// A `DocumentProjectStats` structure containing comprehensive project
    /// document metrics, or a database error if the query fails.
    pub async fn get_project_document_stats(
        ...
        // Total active documents in project
        ...
        // Recently created documents (within 7 days)
        ...
        Ok(DocumentProjectStats {
            total_count,
            archived_count: 0, // Archive functionality not currently implemented
            recent_count,
        })
    }

    /// Gets comprehensive document statistics for a specific user.
    ///
    /// Calculates detailed metrics about a user's document creation activity
    /// and contribution patterns. This provides insights into user engagement,
    /// content creation productivity, and helps identify active contributors
    ...
    /// # Returns
    ///
    /// A `DocumentUserStats` structure containing comprehensive user document
    ...
    pub async fn get_user_document_stats(
        ...
        // Recently created documents (within 30 days)
        let recent_count: i64 = documents::table
            .filter(dsl::account_id.eq(account_id))
...
/// Comprehensive statistics for documents within a project workspace.
///
/// Provides insights into document creation patterns, team collaboration
/// levels, and overall project documentation health. These metrics help
/// project managers understand content creation trends and identify
/// opportunities to improve documentation practices.
#[derive(Debug, Clone, PartialEq)]
pub struct DocumentProjectStats {
    /// Total number of active documents in the project
    pub total_count: i64,
    /// Number of archived documents (currently unused)
    pub archived_count: i64,
    /// Number of documents created in the last 7 days
    pub recent_count: i64,
}

impl DocumentProjectStats {
    /// Calculates the archive rate as a percentage (0-100).
    ///
    /// Returns the percentage of documents that have been archived,
    /// which can indicate content lifecycle management patterns.
    /// Currently returns 0.0 as archive functionality is not implemented.
    ...
    /// Indicates whether the project shows recent content creation activity.
    ///
    /// Returns true if any documents were created in the last 7 days,
    /// which can indicate active project engagement and content development.
    pub fn has_recent_activity(&self) -> bool {
        self.recent_count > 0
    }

    /// Indicates whether the project has any documentation.
    ///
    /// Returns true if the project contains any active documents,
    /// which is useful for identifying projects that need documentation.
    ...
    ///
    /// Returns the percentage of total documents that were created recently,
    /// indicating the pace of new content creation within the project.
...
/// Comprehensive statistics for documents created by a specific user.
///
/// Provides insights into individual user contribution patterns, content
/// creation productivity, and engagement levels within the collaborative
/// environment. These metrics help identify active contributors and
/// understand user participation in documentation efforts.
#[derive(Debug, Clone, PartialEq)]
pub struct DocumentUserStats {
    /// Total number of documents created by the user
    pub total_count: i64,
    /// Number of documents created by the user in the last 30 days
    pub recent_count: i64,
}

impl DocumentUserStats {
    /// Indicates whether the user is actively creating content.
    ///
    /// Returns true if the user has created any documents in the last 30 days,
    /// which can indicate current engagement and contribution levels.
    pub fn is_active_creator(&self) -> bool {
        self.recent_count > 0
    }

    /// Indicates whether the user has created any documents.
    ///
    /// Returns true if the user has created any documents in the system,
    /// which is useful for identifying contributing versus consuming users.
    pub fn has_created_content(&self) -> bool {
        self.total_count > 0
    }

    /// Calculates the recent activity rate as a percentage.
    ///
    /// Returns the percentage of the user's total documents that were
    /// created recently, indicating their current activity level.
    pub fn recent_activity_rate(&self) -> f64 {
        if self.total_count == 0 {
            0.0
        } else {
