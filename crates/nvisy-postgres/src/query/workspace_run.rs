...
use crate::model::{NewWorkspaceRun, WorkspaceRun, UpdateWorkspaceRun};
...
pub trait WorkspaceRunRepository {
    /// Creates a new workspace run with the provided configuration.
    fn create_run(
        &mut self,
        new_run: NewWorkspaceRun,
    ) -> impl Future<Output = PgResult<WorkspaceRun>> + Send;

    /// Finds a run by its unique identifier.
    fn find_run_by_id(
        &mut self,
        run_id: Uuid,
    ) -> impl Future<Output = PgResult<Option<WorkspaceRun>>> + Send;
    ...
    ) -> impl Future<Output = PgResult<Vec<WorkspaceRun>>> + Send;
...
impl WorkspaceRunRepository for PgConnection {
    async fn create_run(&mut self, new_run: NewWorkspaceRun) -> PgResult<WorkspaceRun> {
        use schema::workspace_runs;

        let run = diesel::insert_into(workspace_runs::table)
            .values(&new_run)
            .returning(WorkspaceRun::as_returning())
            .get_result(self)
            .await
    ...
    async fn find_run_by_id(&mut self, run_id: Uuid) -> PgResult<Option<WorkspaceRun>> {
        use schema::workspace_runs::{self, dsl};

        let run = workspace_runs::table
            .filter(dsl::id.eq(run_id))
            .select(WorkspaceRun::as_select())
            .first(self)
            .await
            .optional()
            .map_err(PgError::from)?;
    ...
    async fn find_runs_by_workspace(
        ...
        workspace_id: Uuid,
        pagination: Pagination,
    ) -> PgResult<Vec<WorkspaceRun>> {
        use schema::workspace_runs::{self, dsl};

        let runs = workspace_runs::table
    ...
    async fn find_in_progress_runs(
        ...
        let mut query = workspace_runs::table
    ...
    async fn update_run(
        ...
        run_id: Uuid,
        updates: UpdateWorkspaceRun,
    ) -> PgResult<WorkspaceRun> {
        use schema::workspace_runs::{self, dsl};

        let run = diesel::update(workspace_runs::table.filter(dsl::id.eq(run_id)))
            .set(&updates)
            .returning(WorkspaceRun::as_returning())
            .get_result(self)
            .await
    ...
    async fn mark_run_started(&mut self, run_id: Uuid) -> PgResult<WorkspaceRun> {
        use schema::workspace_runs::{self, dsl};
