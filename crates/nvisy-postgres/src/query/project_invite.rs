...
impl ProjectInviteRepository {
    ...
    /// Retrieves comprehensive invitation statistics for a specific project.
    ///
    /// Compiles detailed metrics about invitation activity for the project,
    /// including counts for different invitation statuses. These statistics
    /// are essential for project management dashboards, invitation effectiveness
    /// analysis, and administrative oversight.
    ///
    /// # Arguments
    ///
    /// * `conn` - Active database connection for the queries
    /// * `proj_id` - UUID of the project to generate statistics for
    ///
    /// # Returns
    ///
    /// A tuple containing `(pending_count, accepted_count, declined_count, expired_count)`,
    /// or a database error if any of the queries fail.
    ///
    /// # Statistics Breakdown
    ///
    /// - `pending_count`: Invitations still awaiting response
    /// - `accepted_count`: Successfully accepted invitations
    /// - `declined_count`: Invitations declined by invitees
    /// - `expired_count`: Invitations that expired without response
    ///
    /// # Project Analytics Use Cases
    ///
    /// - Project growth and recruitment analytics
    /// - Invitation effectiveness measurement
    /// - Member acquisition funnel analysis
    /// - Administrative project health monitoring
