...
pub trait WorkspacePolicyRepository {
    ...
    /// Lists all policies in a workspace with cursor pagination.
    fn cursor_list_workspace_policies(
        &mut self,
        workspace_id: Uuid,
        pagination: CursorPagination,
    ) -> impl Future<Output = PgResult<CursorPage<WorkspacePolicy>>> + Send;
...
impl WorkspacePolicyRepository for PgConnection {
    ...
    async fn cursor_list_workspace_policies(
        ...
        workspace_id: Uuid,
        pagination: CursorPagination,
    ) -> PgResult<CursorPage<WorkspacePolicy>> {
        use schema::workspace_policies::{self, dsl};
        ...
        let items: Vec<WorkspacePolicy> = if let Some(cursor) = &pagination.after {
        ...
        Ok(CursorPage::new(
            ...
            total,
            pagination.limit,
            |p: &WorkspacePolicy| (p.created_at.into(), p.id),
        ))
    }
