//! Files repository for managing uploaded files.
...
use crate::model::{File, NewFile, UpdateFile};
use crate::types::{
...
pub trait FileRepository {
    ...
    fn cursor_list_workspace_files(
        ...
        filter: FileFilter,
    ) -> impl Future<Output = PgResult<CursorPage<File>>> + Send;
...
impl FileRepository for PgConnection {
    ...
    async fn update_file(&mut self, file_id: Uuid, updates: UpdateFile) -> PgResult<File> {
        use schema::files::{self, dsl};

        let file = diesel::update(files::table.filter(dsl::id.eq(file_id)))
            .set(&updates)
            .returning(File::as_returning())
            .get_result(self)
            .await
    ...
    async fn delete_files(&mut self, workspace_id: Uuid, file_ids: &[Uuid]) -> PgResult<usize> {
        use diesel::dsl::now;
        use schema::files::{self, dsl};
    ...
    async fn cursor_list_workspace_files(
        ...
        let mut base_query = files::table
            .filter(dsl::workspace_id.eq(workspace_id))
            .filter(dsl::deleted_at.is_null())
            .into_boxed();
        ...
        let items: Vec<File> = if let Some(cursor) = &pagination.after {
        ...
        Ok(CursorPage::new(
            ...
            total,
            pagination.limit,
            |f: &File| (f.created_at.into(), f.id),
        ))
    }

    async fn find_files_by_hash(&mut self, file_hash: &[u8]) -> PgResult<Vec<File>> {
        use schema::files::{self, dsl};
    ...
    async fn get_account_storage_usage(&mut self, account_id: Uuid) -> PgResult<BigDecimal> {
        ...
        let usage: Option<BigDecimal> = files::table
            .filter(dsl::account_id.eq(account_id))
            .filter(dsl::deleted_at.is_null())
    ...
    async fn list_file_versions(&mut self, file_id: Uuid) -> PgResult<Vec<File>> {
        use schema::files::{self, dsl};
    ...
    async fn get_next_version_number(&mut self, file_id: Uuid) -> PgResult<i32> {
        ...
        let max_version: Option<i32> = files::table
            .filter(dsl::id.eq(file_id).or(dsl::parent_id.eq(file_id)))
            .filter(dsl::deleted_at.is_null())
