//! Project webhook model for PostgreSQL database operations.
//!
//! This module provides models for managing webhooks connected to projects.
//! Webhooks enable projects to send event notifications to external services.

use diesel::prelude::*;
use jiff_diesel::Timestamp;
use uuid::Uuid;

use crate::schema::project_webhooks;
use crate::types::{HasCreatedAt, HasDeletedAt, HasOwnership, HasUpdatedAt, WebhookStatus};

/// Project webhook model representing a webhook configuration for a project.
///
/// This model manages webhook endpoints that receive event notifications from
/// projects. Each webhook maintains its own lifecycle with status tracking,
/// failure handling, and delivery monitoring.
#[derive(Debug, Clone, PartialEq, Queryable, Selectable)]
#[diesel(table_name = project_webhooks)]
...
/// Data structure for creating a new project webhook.
#[derive(Debug, Default, Clone, Insertable)]
#[diesel(table_name = project_webhooks)]
#[diesel(check_for_backend(diesel::pg::Pg))]
pub struct NewProjectWebhook {
    /// Reference to the project this webhook will belong to.
    pub project_id: Uuid,
...
/// Data structure for updating an existing project webhook.
...
impl HasCreatedAt for ProjectWebhook {
    fn created_at(&self) -> jiff::Timestamp {
        self.created_at.into()
    }
}

impl HasUpdatedAt for ProjectWebhook {
    fn updated_at(&self) -> jiff::Timestamp {
        self.updated_at.into()
    }
}

impl HasDeletedAt for ProjectWebhook {
    fn deleted_at(&self) -> Option<jiff::Timestamp> {
        self.deleted_at.map(Into::into)
    }
}

impl HasOwnership for ProjectWebhook {
