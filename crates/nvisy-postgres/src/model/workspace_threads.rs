//! Workspace thread model: a discussion thread — an author, an optional title,
//! an open/closed lifecycle, and a stream of [`WorkspaceThreadComment`] messages
//! and [`WorkspaceThreadEvent`] timeline entries. A thread is not about any
//! resource on its own; a [`WorkspaceReview`] attaches one to a document.
//!
//! [`WorkspaceThreadComment`]: super::WorkspaceThreadComment
//! [`WorkspaceThreadEvent`]: super::WorkspaceThreadEvent
//! [`WorkspaceReview`]: super::WorkspaceReview
...
/// A discussion thread: an author, an optional title, and an open/closed
/// lifecycle. A [`WorkspaceReview`] may attach it to a document review; a
/// thread with no review is a free workspace-level discussion.
///
/// [`WorkspaceReview`]: super::WorkspaceReview
...
impl NewWorkspaceThread {
    /// A minimal thread opened by `author`, for tests.
    #[cfg(any(feature = "test_util", test))]
    pub fn test(workspace_id: Uuid, author_account_id: Uuid) -> Self {
        Self {
            workspace_id,
            author_account_id,
            display_name: None,
