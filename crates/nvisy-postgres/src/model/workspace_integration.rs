//! Workspace integration model for PostgreSQL database operations.
//!
//! This module provides models for managing third-party integrations connected
//! to workspaces. Integrations enable workspaces to connect with external services
//! like version control systems, CI/CD pipelines, monitoring tools, and other
//! development workflow services.
//!
//! ## Models
//!
//! - [`WorkspaceIntegration`] - Main integration model with full configuration and status
//! - [`NewWorkspaceIntegration`] - Data structure for creating new integrations
//! - [`UpdateWorkspaceIntegration`] - Data structure for updating existing integrations

use diesel::prelude::*;
use jiff_diesel::Timestamp;
use uuid::Uuid;

use crate::schema::workspace_integrations;
use crate::types::{
    HasCreatedAt, HasLastActivityAt, HasOwnership, HasUpdatedAt, IntegrationStatus, IntegrationType,
};

/// Workspace integration model representing a third-party integration connected to a workspace.
///
/// This model manages connections between workspaces and external services, storing
...
#[diesel(table_name = workspace_integrations)]
...
/// Data structure for creating a new workspace integration.
///
...
/// Data structure for updating an existing workspace integration.
///
...
impl HasCreatedAt for WorkspaceIntegration {
    fn created_at(&self) -> jiff::Timestamp {
        self.created_at.into()
    }
}

impl HasUpdatedAt for WorkspaceIntegration {
    fn updated_at(&self) -> jiff::Timestamp {
        self.updated_at.into()
    }
}

impl HasLastActivityAt for WorkspaceIntegration {
    fn last_activity_at(&self) -> Option<jiff::Timestamp> {
        self.last_sync_at.map(jiff::Timestamp::from)
    }
}

impl HasOwnership for WorkspaceIntegration {
